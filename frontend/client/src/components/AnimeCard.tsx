...
export default function AnimeCard({ id, title, imageUrl, score, episodes }: AnimeCardProps) {
  ...
  const handleClick = () => {
    setLocation(`/anime/${id}`);
  };

  return (
    <div
      onClick={handleClick}
      className="bg-slate-900 rounded-lg overflow-hidden hover:shadow-lg hover:shadow-blue-500/20 transition-all cursor-pointer border border-slate-800 hover:border-blue-500/50"
    >
      <img
        src={imageUrl}
        alt={title}
